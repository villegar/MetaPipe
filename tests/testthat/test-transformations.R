...
test_that("log transformation works", {
  ...
  result_df <- log_transformation(2^data, "POW_2")
  expect_identical(expected_df, result_df)
  expect_warning(result_df <- log_transformation(data, "POW_2"))
  expect_null(result_df)
  filename <- "HIST_LOG_2_POW_2.png"
...
test_that("power transformation works", {
  ...
  result_df <- power_transformation(sqrt(data), "SQRT_2")
  expect_identical(expected_df, result_df)
  expect_warning(result_df <- power_transformation(data, "SQRT_2"))
  expect_null(result_df)
  filename <- "HIST_POW_2_SQRT_2.png"
  expect_true(file.exists(filename))
  expect_false(dir.exists(filename))
  expect_gt(file.size(filename), 0)
  file.remove(filename)
